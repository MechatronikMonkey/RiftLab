...
.NOTES
    IMPORTANT - order of operations:
      ...
      2. let CI run once, so GitHub has seen the check names `tests` (a required
         check that never reports blocks merging forever, and a typo is
         invisible until a pull request hangs)
      3. run this script
...
[CmdletBinding()]
param(
    [string]$Repo = "MechatronikMonkey/RiftLab",
